...
void render() {
	...
	static int frames = 0;
	++frames;
	static auto last_fps = std::chrono::high_resolution_clock::now();
	auto now = std::chrono::high_resolution_clock::now();
	//static int last_test = 0;
	//static int test = 0;
	if (now-last_fps>=std::chrono::seconds(1)) {
		fps = (double)frames / std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1, 1>>>(now - last_fps).count();
		last_fps = now;
		frames = 0;
	}
