...
enum class pathing_map_index { default_, no_enemy_buildings, include_liftable_wall };
...
struct pathing_map {
	...
	a_vector<std::pair<unit*, unit*>> nydus_canals;
...
pathing_map&get_pathing_map(unit_type*ut, pathing_map_index index = pathing_map_index::default_) {
...
void generate_path_nodes(pathing_map&map) {
	...
	for (auto&v : new_path_nodes) {
		if (v.root_index == (size_t)-1) continue;
	...
	for (size_t i = 0; i < new_path_nodes.size(); ++i) {
		if (new_path_nodes[i].root_index == (size_t)-1) {
...
bool unit_can_reach(unit_type*ut, xy from, xy to, pathing_map_index index = pathing_map_index::default_) {
...
bool unit_can_reach(unit*u, xy from, xy to, pathing_map_index index = pathing_map_index::default_) {
...
double get_distance(pathing_map&map, xy from_pos, xy to_pos) {
	...
	if (map.cached_distance.size() >= 0x10000) map.cached_distance.clear();
...
// 	}
	map.nydus_canals.clear();
	for (unit* u : my_completed_units_of_type[unit_types::nydus_canal]) {
		if (!u->game_unit->getNydusExit()) continue;
		unit* e = units::get_unit(u->game_unit->getNydusExit());
		if (!e) continue;
		map.nydus_canals.emplace_back(u, e);
		//map.nydus_canals.emplace_back(e, u);
	}
}
...
// 	//log("returning %p\n", r);
// 	return r;
	double distance = get_distance(map, from, to);
	unit* best_u = nullptr;
	double best_distance = distance;
	for (auto& v : map.nydus_canals) {
		unit* u = v.first;
		unit* e = v.second;

		double distance_to_u = get_distance(map, from, u->pos);
		double distance_from_e = get_distance(map, e->pos, to);
		if (distance_to_u + distance_from_e < best_distance) {
			best_distance = distance_to_u + distance_from_e;
			best_u = u;
		}
...
// 	auto&test = get_pathing_map(unit_types::dragoon);
