...
struct strat_z_3hatch_spire : strat_z_base {
...
// 		};
		...
		defence_fight_ok = eval_combat(true, my_workers.size() < 16 ? 0 : 1);
		...
		resource_gathering::max_gas = 200;
		if (spire_progress) resource_gathering::max_gas = 900;

		if (my_units_of_type[unit_types::mutalisk].size() >= 10) has_built_mutas = true;
...
// 		};
		...
		bool build_army = is_defending && (sunken_count == 0 || !defence_fight_ok || army_supply < (players::opponent_player->race == race_protoss ? 3.0 : 2.0));
		//if (being_rushed && drone_count >= 13 && army_supply < 8) build_army = true;
		if (army_supply < enemy_attacking_army_supply - sunken_count * 4) build_army = true;
		...
		if (spire_progress >= 1.0 && count_production(st, unit_types::drone)) {
		...
		if ((!is_defending && !being_rushed) || defence_fight_ok) {
			if (drone_count >= 13 && count_units_plus_production(st, unit_types::extractor) < (st.units[unit_types::lair].empty() ? 1 : 2)) {
				army = [army](state&st) {
...
//		}
		...
		if (my_completed_units_of_type[unit_types::hatchery].size() >= 2) {
			...
			if (maybe_being_rushed && sunken_count < (drone_count >= 24 ? 3 : 2)) {
				army = [army](state&st) {
					return nodelay(st, unit_types::sunken_colony, army);
				};
			}
