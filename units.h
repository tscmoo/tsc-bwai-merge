...
a_unordered_map<unit_type*, int> total_units_made_of_type;
...
struct unit {
	...
	int last_incoming_bullet = 0;
	unit* last_incoming_bullet_from = nullptr;
	int last_lost_hp = 0;
	...
	bool temporary_combat_flag;
	bool is_in_anti_push_group = false;
...
namespace unit_types {
	a_vector<unit_type*> all;
	void get(unit_type*&rv,BWAPI::UnitType game_unit_type) {
		all.push_back(units::get_unit_type(rv, game_unit_type));
	}
	...
	unit_type_pointer missile_turret, academy, comsat_station, starport, control_tower, engineering_bay, armory, covert_ops, physics_lab;
	...
	unit_type_pointer zealot, dragoon, dark_templar, high_templar, reaver, scarab;
	...
	unit_type_pointer hydralisk_den, spire, greater_spire, extractor, ultralisk_cavern, defiler_mound, queens_nest;
	...
	void init() {
		...
		get(physics_lab, BWAPI::UnitTypes::Terran_Physics_Lab);
		...
		get(scarab, BWAPI::UnitTypes::Protoss_Scarab);
		...
		get(queens_nest, BWAPI::UnitTypes::Zerg_Queens_Nest);
...
std::tuple<double, double> get_unit_value(unit_type*from, unit_type*to) {
	double minerals = 0.0;
	double gas = 0.0;
	if (from != nullptr && from == unit_types::siege_tank_tank_mode && to == unit_types::siege_tank_siege_mode) return std::make_tuple(minerals, gas);
	if (from != nullptr && from == unit_types::siege_tank_siege_mode && to == unit_types::siege_tank_tank_mode) return std::make_tuple(minerals, gas);
	...
	if (to->race == race_zerg) {
		...
		if (from != nullptr && from == unit_types::siege_tank_siege_mode) from = unit_types::siege_tank_tank_mode;
...
void update_unit_type(unit*u) {
	...
	if (ut->is_building || (u->owner != players::my_player && (u->type == unit_types::egg || u->type == unit_types::lurker_egg))) {
		if (!u->building) {
...
void update_weapon_stats(weapon_stats*st) {
	...
	st->cooldown = gw.damageCooldown();
	if (gw == BWAPI::WeaponTypes::Claws && gp->getUpgradeLevel(BWAPI::UpgradeTypes::Adrenal_Glands)) st->cooldown = 5;
	st->min_range = gw.minRange();
	st->max_range = gp->weaponMaxRange(gw);
	if (gw == BWAPI::WeaponTypes::Scarab) st->max_range = 32 * 8;
...
void update_stats(unit_stats*st) {
	...
	st->last_update = current_frame;

	st->max_speed = std::ceil(gp->topSpeed(gu) * 256) / 256.0;
	if (st->type->is_worker) st->max_speed = 5.0;
...
void update_unit_stuff(unit*u) {
	...
	if (u->visible != u->game_unit->isVisible(players::self)) xcept("visible mismatch in unit %s (%d vs %d)", u->type->name, u->visible, u->game_unit->isVisible(players::self));
	...
	bool burrowing = u->game_order == BWAPI::Orders::Burrowing;
	if (burrowing && !u->burrowing) u->burrowing_start_frame = current_frame;
	u->burrowing = burrowing;
	u->burrowed = u->game_unit->isBurrowed() || burrowing;
	...
	if (!u->cloaked || u->detected) {
		...
		if (u->hp < last_hp) u->last_lost_hp = current_frame;
	}
	...
	if (u->game_order == BWAPI::Orders::AttackUnit || u->last_attacked == current_frame) {
		u->last_attack_target = u->order_target;
	}
	...
	if (u->game_unit->isLockedDown() && u->lockdown_timer == 0) u->lockdown_timer = 90;
	u->maelstrom_timer = u->game_unit->getMaelstromTimer() * 8;
	if (u->game_unit->isMaelstrommed() && u->maelstrom_timer == 0) u->maelstrom_timer = 90;
	u->stasis_timer = u->game_unit->getStasisTimer() * 8;
	if (u->game_unit->isStasised() && u->stasis_timer == 0) u->stasis_timer = 90;
	...
	u->has_parasite = u->game_unit->isParasited();
...
// 		if ((size_t)tile_pos.x + u->type->tile_width > (size_t)grid::build_grid_width || (size_t)tile_pos.y + u->type->tile_height > (size_t)grid::build_grid_height) xcept("unit %s has tile pos outside map", u->type->name);
	...
	if (current_frame - u->stats->last_update >= 15 * 5) {
...
unit*new_unit(BWAPI_Unit game_unit) {
	...
	u->hp = u->stats->hp;
	u->visible = game_unit->isVisible(players::self);
	u->pos = {0, 0};
	update_unit_stuff(u);
...
void update_projectile_stuff_task() {
	while (true) {
		...
		for (BWAPI_Bullet b : game->getBullets()) {
			unit* u = targetunit(b->getSource());
			unit* target = targetunit(b->getTarget());
			if (target) {
				target->last_incoming_bullet = current_frame;
				target->last_incoming_bullet_from = u;
				xy target_pos = bwapi_pos(b->getTargetPosition());
				if ((!u || u->owner->is_enemy) && target->cloaked && !combat::is_in_detected_area(target_pos)) combat::add_detected_area(target_pos);
			}
...
a_vector<std::function<void(unit*)>> on_create_callbacks, on_morph_callbacks, on_destroy_callbacks, on_show_callbacks;
...
void update_units_task() {
	...
	a_vector<unit*> created_units, morphed_units, destroyed_units, shown_units;
	...
	auto update_groups = [](unit*u) {
		...
		//bool has_ever_been_shown = u->last_shown != 0;
		bool has_ever_been_shown = true;

		update_unit_container(u, enemy_units, !u->dead && u->owner->is_enemy && has_ever_been_shown);
		update_unit_container(u, visible_enemy_units, u->visible && u->owner->is_enemy && has_ever_been_shown);
		update_unit_container(u, enemy_buildings, !u->dead && !u->gone && u->building && u->owner->is_enemy && has_ever_been_shown);
		update_unit_container(u, enemy_detector_units, !u->dead && u->owner->is_enemy && u->type->is_detector && has_ever_been_shown);
	...
	while (true) {
		...
		if (game->isReplay()) {
			for (auto&game_unit : game->getAllUnits()) {
				unit*u = get_unit(game_unit);
				if (!u) continue;
				if (!u->visible) {
					if (game_unit->isVisible(players::self)) events.emplace_back(event_t::t_show, game_unit);
				} else {
					if (!game_unit->isVisible(players::self)) events.emplace_back(event_t::t_hide, game_unit);
				}
			}
		}
		...
		while (!events.empty()) {
			...
			u->visible = u->game_unit->isVisible(players::self);
			...
			case event_t::t_create:
				...
				if (u->owner == players::my_player) ++total_units_made_of_type[u->type];
				break;
...
//						log("%s is gone\n", u->type->name);
//						//printf("%s is gone - %d %d %d\n", u->type->name.c_str(), is_visible_around(u->pos), u->burrowed, has_detection_at(u->pos));
//						u->gone = true;
//						events.emplace_back(event_t::t_refresh, u->game_unit);
//					}
//				}
...
//		}
		...
		for (unit* u : shown_units) {
			for (auto& f : on_show_callbacks) f(u);
		}
		shown_units.clear();
...
//	a_unordered_map<unit_type*, int> counts;
//
//	int my_live_units = 0;
//	int my_dead_units = 0;
//	int op_live_units = 0;
...
//			if (!u->dead) ++my_live_units;
//			else ++my_dead_units;
//		} else if (u->owner->is_enemy) {
//			if (!u->dead) ++op_live_units;
//			else ++op_dead_units;
//		}
//	}
//
//	a_unordered_set<unit*> added;
//	for (unit* u : enemy_units) {
//		if (u->dead) DebugBreak();
//		if (!added.insert(u).second) DebugBreak();
//
//		if (!u->type->is_worker && !u->building) {
//			++counts[u->type];
//		}
//	}

// 	log(log_level_test, " -- %d %d %d %d\n", my_live_units, my_dead_units, op_live_units, op_dead_units);
// 
// 	for (auto& v : counts) {
// 		log(log_level_test, " %d x %s\n", v.second, v.first->name);
// 	}
