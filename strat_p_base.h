...
struct strat_p_base {
	...
	std::function<refcounted_ptr<resource_spots::spot>()> get_next_base = [this]() {
		...
		return get_best_score(available_bases, [&](resource_spots::spot*s) {
			double score = 0;
			//if (st.bases.size() == 1) score += unit_pathing_distance(worker_type, s->cc_build_pos, st.bases.front().s->cc_build_pos);
...
//		combat::aggressive_corsairs = false;
		...
		auto place_expos = [&]() {
			...
			for (auto&b : build::build_tasks) {
				...
				if (b.type->unit && b.type->unit->is_resource_depot && b.type->unit->builder_type->is_worker) {
					...
					else log(log_level_test, "get_next_base() failed\n");
					log(log_level_test, " place expos at %d %d\n", pos.x, pos.y);
		...
		while (true) {
			...
			for (auto&s : resource_spots::spots) {
				...
				if (bs.building && bs.building->owner == players::opponent_player) ++enemy_bases;
