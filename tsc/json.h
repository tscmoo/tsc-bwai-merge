...
struct json_value {
	...
	const char*parse(const char*str,size_t len) {
		...
		auto parse_as_number = [&]() -> const char* {
			...
			auto put = [&]() {
				char c = *str++;
				if (o + 1 >= buffer.data() + buffer.size()) {
					size_t offset = o - buffer.data();
					buffer.resize(buffer.size() + 0x10 + buffer.size() / 2);
					o = buffer.data() + offset;
				}
				*o++ = c;
			};
			auto digit = [&]() {
	...
	a_string dump(const char* num_format = "%.100g") const {
		...
		auto num = [&]() -> a_string {
			...
			snprintf(buf, 0x100, num_format, n);
			return buf;
		};
