...
struct scout {
	...
	enum { type_default, type_scout_proxy, type_base_scout };
	...
	xy wait_pos;
	bool is_first_scout = false;
	int last_check_valid_path = 0;
...
// 	}
	...
	if (type == type_scout_proxy) {
		...
		if (current_frame >= 15 * 60 * 4 + 15 * 30) {
	...
	auto dodge_enemies = [&]() {
		if (!combat::entire_threat_area.test(grid::build_square_index(scout_unit->pos))) return;
		xy target_pos = scout_unit->controller->go_to;
		int edge_n = 0;
		xy best_pos;
		double best_d = std::numeric_limits<double>::infinity();
		a_deque<xy> path = combat::find_path(scout_unit->type, scout_unit->pos, [&](xy pos, xy npos) {
			return diag_distance(npos - scout_unit->pos) <= 32 * 10;
		}, [&](xy pos, xy npos) {
			double cost = 0.0;
			...
			//if (combat::entire_threat_area_edge.test(index)) {
				//++edge_n;
			if (true) {
				double d = diag_distance(pos - target_pos);
				if (d < best_d) {
					best_d = d;
					best_pos = pos;
				}
				//if (edge_n >= 4) return true;
			}
			return false;
		});

		if (!path.empty()) scout_unit->controller->go_to = square_pathing::get_go_to_along_path(scout_unit, path);
		//if (!path.empty()) scout_unit->controller->go_to = path.back();
		//if (best_pos != xy()) scout_unit->controller->go_to = best_pos;
	};

	if (type == type_base_scout) {
		...
		if (found_them && combat::my_closest_base != xy() && combat::op_closest_base != xy()) {
			...
			if (timer <= current_frame || (current_frame - timer <= 20 && diag_distance(scout_unit->pos - scout_unit->controller->go_to) <= 32 * 4)) {
				...
				xy dst;
				//combat::find_nearby_entirely_walkable_tiles(scout_unit->pos, [&](xy pos) {
				combat::find_nearby_entirely_walkable_tiles(op_start_loc, [&](xy pos) {
					if (dst != xy()) return false;
					auto&bs = grid::get_build_square(pos);
					if (bs.building) return false;
					if (current_frame - bs.last_seen >= 15 * 60 * 3) dst = pos;
				...
				if (dst != xy()) {
					...
					scout_unit->controller->action = unit_controller::action_idle;
					unit* target = get_best_score(enemy_buildings, [&](unit* u) {
						return u->last_seen;
					});
					if (target) {
						scout_unit->controller->action = unit_controller::action_scout;
						scout_unit->controller->go_to = target->pos;
					}
					if (s) {
						resource_spots::spot* expo_s = get_best_score_p(resource_spots::spots, [&](resource_spots::spot* ns) {
							if (ns == s) return std::numeric_limits<double>::infinity();
							return unit_pathing_distance(unit_types::scv, op_start_loc, ns->pos);
						});
						if (expo_s && current_frame - grid::get_build_square(expo_s->pos).last_seen > 15 * 30) {
							scout_unit->controller->action = unit_controller::action_scout;
							scout_unit->controller->go_to = expo_s->pos;
						}
					}
					if (scout_unit->controller->action == unit_controller::action_idle) {
						scout_unit = nullptr;
					}
				}
				if (scout_unit) dodge_enemies();
				return;

// 				if (wait_pos == xy()) {
// 					//auto base_path = combat::find_bigstep_path(scout_unit->type, combat::my_closest_base, combat::op_closest_base, square_pathing::pathing_map_index::no_enemy_buildings);
// 
// 					struct node_data_t {
// 						double score = 0;
// 					};
...
// 					xy best_pos;
// 					combat::find_path<node_data_t>(scout_unit->type, combat::op_closest_base, [&](xy pos, xy npos, node_data_t&n) {
// 						if (combat::entire_threat_area.test(grid::build_square_index(npos))) n.score += 64.0;
// 						n.score -= diag_distance(npos - pos);
// 						if (n.score < best_score) {
// 							best_score = n.score;
// 							best_pos = npos;
// 						}
// 						return diag_distance(npos - combat::op_closest_base) <= 32 * 20;
...
// 					wait_pos = best_pos;
// 
// 					log(log_level_info, " wait_pos is %d %d\n", wait_pos.x, wait_pos.y);
// 				}
// 
// 				if (current_frame < 15*60*6) {
// 					log(log_level_info, " scout base!\n");
// 					if (wait_pos != xy()) {
// 						scout_unit->controller->action = unit_controller::action_scout;
// 						scout_unit->controller->go_to = wait_pos;
// 					}
// 
// 					dodge_enemies();
// 					return;
...
// 				}
	...
	if (current_used_total_supply < 30 && is_first_scout) {
		bool found_them = test_pred(enemy_buildings, [&](unit*e) {
			return e->type->is_resource_depot;
		});
		if (found_them && combat::my_closest_base != xy() && combat::op_closest_base != xy()) {
			...
			if (timer <= current_frame || (current_frame - timer <= 20 && diag_distance(scout_unit->pos - scout_unit->controller->go_to) <= 32 * 2)) {
				...
				if (current_frame > 15 * 60 * 5 && current_frame < 15 * 60 * 7) {
					...
					if (path.size() >= 4) {
						...
						for (unit* u : visible_enemy_units) {
							...
							for (size_t i = 0; i != path.size(); ++i) {
								xy pos = path[i];
								double d = diag_distance(u->pos - pos);
								if (d < best_d) {
									best_d = d;
									best_index = i;
								}
							}
							if (best_index != 0 && (nearest_index == 0 || best_index < nearest_index)) nearest_index = best_index;
						}
						
						if (nearest_index != 0) {
							scout_unit->controller->action = unit_controller::action_scout;
							scout_unit->controller->go_to = path[nearest_index - 1];
							return;
						}
	...
	auto check_path = [&](xy to) {
		if (current_frame < 15 * 60 * 4) return true;
		if (!square_pathing::unit_can_reach(scout_unit, scout_unit->pos, to)) return false;
		for (auto* n : square_pathing::find_path(square_pathing::get_pathing_map(scout_unit->type), scout_unit->pos, to)) {
			if (!combat::can_transfer_through(n->pos)) return false;
		}
		return true;
	};
	
	if (dst_s && current_frame - last_check_valid_path >= 15) {
		if (!check_path(dst_s->pos)) dst_s = nullptr;
	}
		...
		dst_s = get_best_score_p(resource_spots::spots, [&](resource_spots::spot*s) {
			...
			if (!enemy_buildings.empty()) {
				d = get_best_score_value(enemy_buildings, [&](unit*u) {
					...
					return diag_distance(u->pos - s->cc_build_pos);
				});
			} else {
			...
			if (!scout_unit->is_flying) {
				if (!check_path(s->pos)) return std::numeric_limits<double>::infinity();
...
void scan() {
	...
	int min_scans = 1;
	if (current_used_total_supply >= 90.0) min_scans = 3;
	...
	if (enemy_has_cloaked_units) {
		min_scans = my_completed_units_of_type[unit_types::comsat_station].size() * 2;
	}

	//if (scans_available > 2 || overlords_available > 2 || observers_available > 2) {
	if (scans_available > min_scans || overlords_available > 2 || observers_available > 2) {
		for (auto&s : resource_spots::spots) {
...
// 	}
	...
	if (!has_found_their_start_location) {
		...
		for (auto& s : resource_spots::spots) {
			xy pos = s.pos;
			double d = diag_distance(pos - op_start_loc);
			int last_seen = grid::get_build_square(pos).last_seen;
			int age = current_frame - last_seen;
			if (age <= 15 * 60 * 4) continue;
			values[pos] += age / (d / (32 * 4));
		}
	}
...
bool next_scout_is_first = true;

scout&add_scout(unit*u) {
	scout sc;
	sc.scout_unit = u;
	sc.is_first_scout = next_scout_is_first;
	if (next_scout_is_first) next_scout_is_first = false;
...
//bool next_is_base_scout = true;
bool next_is_base_scout = false;
bool sent_5min_scout = true;
	...
	if (all_scouts.empty()) {
		if (last_scout == 0 || current_frame - last_scout >= 15 * 60 * 3 || current_used_total_supply >= 100) {
			if (current_used_total_supply >= scout_supply) {
				...
				if (scout_unit) {
					auto&s = add_scout(scout_unit);
// 					if (next_is_base_scout) {
// 						next_is_base_scout = false;
// 						s.type = scout::type_base_scout;
// 					}
				}
			}
		}
		...
		if (next_is_base_scout) {
			if (!enemy_buildings.empty()) {
				scout* s = get_best_score_p(all_scouts, [&](scout* s) {
					if (!s->scout_unit) return std::numeric_limits<double>::infinity();
					return units_distance(s->scout_unit, enemy_buildings.front());
				}, std::numeric_limits<double>::infinity());
				if (s) {
					next_is_base_scout = false;
					s->type = scout::type_base_scout;
...
// 	if (current_frame < 15 * 60 * 16 && current_frame <= 15 * 60 * ((int)my_workers.size() / 2)) {
// 		if (current_frame >= 15 * 60 * 4 && current_frame - last_proxy_scout >= 15 * 60 * 2) {
// 			last_proxy_scout = current_frame;
// 			unit*scout_unit = get_best_score(my_workers, [&](unit*u) {
// 				if (u->controller->action != unit_controller::action_gather) return std::numeric_limits<double>::infinity();
// 				return 0.0;
// 			}, std::numeric_limits<double>::infinity());
// 			if (scout_unit) add_scout(scout_unit).type = scout::type_default;
// 		}
// 	}
