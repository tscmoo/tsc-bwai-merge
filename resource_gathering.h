...
void process(gatherer_t&g) {
	...
	if (g.last_find_transfer == 0 || current_frame - g.last_find_transfer >= 15 * 5 || delivered || relocate) {
		...
		if (!g.u->is_flying) {
			for (auto&r : live_resources) {
				...
				for (auto*n : square_pathing::find_path(square_pathing::get_pathing_map(g.u->type), g.u->pos, r.u->pos)) {
					//xy pos = n->pos;
					if (my_resource_depots.size() >= 2 && !combat::can_transfer_through(n->pos)) {
						can_reach = false;
						break;
					}
				}
		...
		resource_t*new_r = get_best_score_p(live_resources, [&](resource_t*r) {
			...
			if (r->u->type->is_gas && gas_limit >= 350 && current_gas >= 300 && current_minerals < 150 && !r->gatherers.empty()) return 0.0;
			double next_income = 0.0;
			if (r->income_rate.size() > r->gatherers.size()) {
				next_income = r->income_rate[r->gatherers.size()];
				if (r->gatherers.size() > 0) next_income /= 2;
			}
