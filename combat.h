...
//		}
	...
	if (target && ((target->cloaked && !target->detected) || (!target->visible && units_distance(a->u, target) <= 32 * 12)) && (target->is_flying || !wants_to_lay_spider_mines || target->type == unit_types::lurker) && !a->u->type->is_detector && !a->u->is_loaded) {
		bool ignore = false;
		if (!target->cloaked) {
		...
		if (!target->game_unit->isVisible() || target->game_unit->isDetected()) ignore = true;
...
// 		a->target_pos = best_pos;
	if (false) {
		...
		if (true) {
			...
			a->target_pos = path.empty() ? a->goal_pos : path.back();
			if (no_aggressive_groups && path.empty()) {
