...
struct strat_z_9pool_speed_into_1hatch_spire : strat_z_base {
	...
	virtual bool build(buildpred::state&st) override {
		...
		if (count_units_plus_production(st, unit_types::spire) && enemy_lair_count + enemy_spire_count == 0) {
			if (sunken_count < 2) {
				army = [army](state&st) {
					return nodelay(st, unit_types::sunken_colony, army);
				};
		...
		if (enemy_zealot_count + enemy_marine_count / 2 > count_units_plus_production(st, unit_types::sunken_colony)) {
			if (sunken_count < 10) {
				army = [army](state&st) {
					return nodelay(st, unit_types::sunken_colony, army);
				};
			}
			if (sunken_count < 2 && zergling_count < 6 && is_defending) {
				army = [army](state&st) {
					return nodelay(st, unit_types::zergling, army);
				};
			}
