...
build_square* build_square::get_neighbor(int d) {
	xy p;
...
struct build_grid_indexer {
	size_t operator()(xy pos) const {
		return (unsigned)pos.x / 32 + (unsigned)pos.y / 32 * (size_t)build_grid_width;
	}
};
...
template<typename T = unit_type>
void reserve_build_squares(xy pos,T*ut,bool reserve=true) {
	int w = ut->tile_width;
...
void init() {
		...
		for (int x = 0; x < build_grid_width; ++x) {
			...
			for (int yy = 0; yy < 4 && bs.entirely_walkable; ++yy) {
				for (int xx = 0; xx < 4 && bs.entirely_walkable; ++xx) {
					...
					else bs.partially_walkable = true;
				}
			}
		}
	}
	...
	multitasking::spawn(update_mineral_reserved_task,"update mineral reserved");
