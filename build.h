...
struct build_task: refcounted {
	...
	int fail_count = 0;
	bool skip_pylon_creep_check = false;
};
...
build_type* get_build_type(unit_type* unit) {
	...
	if (unit == unit_types::lurker) r->prerequisites.push_back(get_build_type(upgrade_types::lurker_aspect));
	r->name = unit->name;
	return r;
}
...
build_type* get_build_type(upgrade_type* upgrade) {
	...
	if (upgrade->level > 1) r->prerequisites.push_back(get_build_type(upgrades::get_upgrade_type(upgrade->game_upgrade_type, upgrade->level - 1)));
	r->name = upgrade->name;
	return r;
}
...
bool never_add_pylons_or_creep = false;
...
build_task*get_prerequisite_if_needed(build_task*t,build_type*type) {
	...
	if (type->upgrade) {
		if (players::my_player->upgrades.count(type->upgrade)) return nullptr;
	}
...
void add_build_sum(double priority, upgrade_type*type, int count, void* flag = nullptr) {
	int sum = 0;
	for (build::build_task&b : build::build_tasks) {
		if (b.dead) continue;
		if (b.type->upgrade == type) ++sum;
	}
	for (; sum < count; ++sum) add_build_task(priority, type)->flag = flag;
}
...
// 					}
					...
					for (unit*u : my_units_of_type[b.type->builder]) {
						...
						int busy_time = u->remaining_whatever_time;
						if (u->type == unit_types::hatchery || u->type == unit_types::lair || u->type == unit_types::hive) {
							busy_time = std::max(u->remaining_build_time, std::max(u->remaining_research_time, u->remaining_upgrade_time));
						}
						if (build_delay == -1 || busy_time < build_delay) build_delay = busy_time;
						if (build_delay == 0) {
							break;
						}
					}
...
bool build_is_valid(grid::build_square&bs, unit_type*ut, bool skip_pylon_creep_check = false) {
	bool okay = build_spot_finder::is_buildable_at(ut, bs);
	if (skip_pylon_creep_check) return okay;
	if (!ut->requires_creep && ut != unit_types::hatchery && okay) okay &= build_has_not_creep(bs, ut);
	if (ut->requires_creep && okay) okay &= build_has_creep(bs, ut);
	if (ut->requires_pylon && okay) okay &= build_has_pylon(bs, ut);
	return okay;
...
bool set_build_pos(build_task*t, xy pos) {
	...
	bool okay = build_is_valid(grid::get_build_square(pos), t->type->unit, t->skip_pylon_creep_check);
	if (!okay) {
...
void execute_build(build_task&b) {
	...
	if (b.type->unit && b.type->unit->is_building && !b.built_unit && b.build_frame && (current_frame - b.build_frame <= 15 * 30 || b.send_builder_immediately) && !b.type->unit->is_addon && b.type->builder->is_worker) {
		...
		if (b.build_pos == xy() && !b.dont_find_build_pos) {
			...
			a_vector<xy> starts;
			starts.push_back(b.build_near == xy() ? default_build_pos : b.build_near);
			//if (b.build_near == xy() && my_resource_depots.size() >= 3 && players::my_player->race != race_zerg) {
			if (b.build_near == xy()) {
				if (players::my_player->race == race_protoss && players::opponent_player->race != race_terran) {
//					for (unit* u : my_resource_depots) {
//						if (combat::can_transfer_through(u->pos)) {
//							starts.push_back(u->building->build_pos);
//						}
//					}
				} else if (my_resource_depots.size() >= (players::my_player->race == race_zerg ? 4 : 3)) {
					for (unit* u : my_resource_depots) {
						if (combat::can_transfer_through(u->pos)) {
							if (u->building->build_pos != starts[0]) starts.push_back(u->building->build_pos);
						}
					}
				}
			}
			...
			if (b.type->unit->requires_pylon) {
				...
				if (!never_add_pylons_or_creep) {
					if (pos == xy() && !b.needs_pylon && my_units_of_type[unit_types::pylon].size() > 2) {
		...
		if (!b.built_unit && b.type->unit) {
			...
			if (b.build_frame - current_frame <= 15 * 30) {
				...
				if ((!builder || current_frame - b.last_look_for_builder >= 15 * 2) && current_frame - b.last_look_for_builder >= 15 * 2) {
					...
					builder = get_best_score(my_completed_units_of_type[b.type->unit->builder_type],[&](unit*u) {
						...
						if (b.type->unit->is_worker) {
							int workers_nearby = 0;
							for (unit* w : my_workers) {
								if (diag_distance(w->pos - u->pos) <= 32 * 10) ++workers_nearby;
							}
							resource_spots::spot* s = get_best_score_p(resource_spots::spots, [&](resource_spots::spot* s) {
								return diag_distance(u->pos - s->pos);
							});
							if (s && diag_distance(u->pos - s->pos) <= 32 * 10) {
								double max_workers = s->total_workers[u->type->race];
								cost -= 15 * (max_workers - workers_nearby);
							}
						}
			...
			if (builder && !b.built_unit) {
				if (b.type->unit && !b.type->unit->is_building) {
					...
					unit_building*building = builder->building;
					if (building) {
						if (!building->is_lifted && my_units_of_type[b.type->unit].empty()) {
							if (current_frame + latency_frames >= b.build_frame) {
								if (b.fail_count < 15) {
...
bool match_new_unit(unit*u) {
	...
	for (build_task&b : build_tasks) {
		...
		if (b.build_pos != xy()) {
			...
			if (u->building && diag_distance(u->building->build_pos - b.build_pos) <= 32 * 12) {
				b.built_unit = u;
				b.build_started_frame = current_frame;
				return true;
			}
...
void execute_build_task() {
	while (true) {
		...
		for (auto i = build_tasks.begin(); i != build_tasks.end();) {
			build_task&b = *i++;
			if (b.upgrade_done_frame) {
				if (current_frame >= b.upgrade_done_frame - b.type->build_time + 30) {
					if (b.type->upgrade && !players::my_player->game_player->isUpgrading(b.type->upgrade->game_upgrade_type) && !players::my_player->game_player->isResearching(b.type->upgrade->game_tech_type)) {
						log("upgrade %s is not upgrading, cancelling\n");
						cancel_build_task(&b);
						continue;
					}
				}
				if (current_frame >= b.upgrade_done_frame) {
		...
		for (unit*u : my_buildings) {
			if (!u->is_completed && !u->is_morphing) {
				...
				if (!found && current_frame - u->creation_frame >= 30) {
