...
struct transform_filter {
	...
	template<typename iterator_T>
	struct iterator_impl {
		typedef decltype(std::declval<const transform_filter>().filter(std::declval<typename iterator_T::reference>())) value_type;
...
struct iterators_range {
	...
	size_t size() {
		return (size_t)std::distance(begin_it, end_it);
	}
