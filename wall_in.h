...
void generate(unit_type*path_ut, wall_spot&ws) {
				...
				if (!nbs->entirely_walkable) {
					...
					for (auto*en : edges) {
						...
						if (path.empty()) {
							...
							ws.valid = true;
							log(log_level_test, "generated spot after %d iterations\n", iterations);
							return;
						}
					}
					edges.insert(cn);
	...
	log(log_level_test, "failed to generate spot after %d iterations\n", iterations);
}
	...
	for (size_t i = 0; i + n_space * 2 < path.size(); ++i) {
		...
		if (ws.valid) {
			...
			if (ws.valid) {
				...
				log(log_level_test, "found valid spot with score %g\n", s);
				if (s < best_score) {
					best_score = s;
					best = std::move(ws);
				}
			}
		} else log(log_level_test, "not valid :(\n");
	}
	log(log_level_test, "find_wall_spot_from_to done\n");
	best.path = path;
	return best;
}
...
struct wall_builder {
	...
	bool find() {
		...
		for (size_t i = 0; i < buildings_n; ++i) {
			...
			log(log_level_test, "%d: %d build spots\n", i, vec.size());
		...
		while (true) {
			...
			if (n == buildings_n - 1) {
				...
				multitasking::yield_point();
				//log(log_level_test, "path.size() is %d\n", path.size());
				if (path.empty()) {
		...
		if (found) log(log_level_test, "wall_builder::find succeeded\n");
		else log(log_level_test, "wall_builder::find failed\n");
		log(log_level_test, "wall_builder::find: %d iterations\n", iterations);
	...
	bool build() {
		...
		for (auto&b : build::build_tasks) {
			for (size_t i = 0; i < buildings_pos.size(); ++i) {
				if (b.build_pos == buildings_pos[i]) {
					log(log_level_test, "found wall %d\n", i);
					found[i] = true;
					build_task_taken.insert(&b);
					if (b.built_unit) log(log_level_test, "%s->is_liftable is %d\n", b.built_unit->type->name, b.built_unit->type->is_liftable);
					if (b.built_unit && b.built_unit->type->is_liftable) {
						...
						log(log_level_test, "%s marked as liftable wall\n", b.built_unit->type->name);
					}
					break;
				}
			}
		...
		for (size_t i = 0; i < buildings_pos.size(); ++i) {
			...
			for (auto&b : build::build_tasks) {
				...
				build_task_taken.insert(&b);
				log(log_level_test, "build wall %d!\n", i);
				break;
			}
		}
		log(log_level_test, "remaining is %d\n", remaining);
			...
			if (u->building->is_liftable_wall) {
				...
				if (lift || low_prio_lift) {
					...
					if (!u->building->is_lifted) {
						...
						eval.run();
						log(log_level_test, "lift - my_army %g op_army %g - scores %g %g\n", my_army, op_army, eval.teams[0].score, eval.teams[1].score);
